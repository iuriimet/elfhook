...
ElfFuzz::ElfFuzz(const string& fuzz_so, const string& fuzz_proc) : m_fuzz_so(fuzz_so), m_fuzz_proc(fuzz_proc)
{
    m_elf = elfmem_create();
    if (!m_elf) {
        throw runtime_error(string("Can't create elfmem object"));
    }
    m_fuzz_proc_addr = elfmem_find_sym(m_elf, m_fuzz_so.c_str(), m_fuzz_proc.c_str());
    if (!m_fuzz_proc_addr) {
        throw runtime_error(string("Can't find proc " + m_fuzz_proc + " in " + m_fuzz_so));
    }
    LOG_D("ElfFuzz for %s in %s at %p", m_fuzz_proc.c_str(), m_fuzz_so.c_str(), m_fuzz_proc_addr);
}

ElfFuzz::ElfFuzz(ElfFuzz&& obj) :
    m_fuzz_so(obj.m_fuzz_so), m_fuzz_proc(obj.m_fuzz_proc), m_elf(obj.m_elf), m_fuzz_proc_addr(obj.m_fuzz_proc_addr), m_hook_map(move(obj.m_hook_map))
{
    obj.m_elf = nullptr;
}
ElfFuzz& ElfFuzz::operator=(ElfFuzz&& obj)
{
    ...
    m_fuzz_proc = obj.m_fuzz_proc;
    m_elf = obj.m_elf;
    m_fuzz_proc_addr = obj.m_fuzz_proc_addr;
    m_hook_map = move(obj.m_hook_map);
    obj.m_elf = nullptr;
    return *this;
}
...
ElfFuzz::~ElfFuzz()
{
    ...
    if (m_elf) {
        ...
        LOG_D("ElfFuzz for %s in %s at %p removed", m_fuzz_proc.c_str(), m_fuzz_so.c_str(), m_fuzz_proc_addr);
    }
}
